...
One drawback about it is that it might consume between 2-12 clock cycles depending on the number of leading ones and zeroes in the input operands. In fact, ARM cortex could possibly find a way to optimize the computations depending on the input values.
...
Divsion must never be ommitted, if there is no division hardware support, the compiler would in this case generate "a list of instruction" as a software solution tackling this challenge. The idea was to find a way to transform the dividion operation into a multuplication operation indroducing a clever solution that might be refrenced as "fixed point multuplication". ( At first it might be obvious, dividing by 5 is the same as multiplying with 0.2)

### 3.1 fixed point representation
A “binary point” can be created using our binary representation and the same decimal point concept. A binary point, like in the decimal system (just the same binary representation we are used to), represents the coefficient of the expression 20 = 1. The weight of each digit (or bit) to the left of the binary point is 2^0, 2^1, 2^2, and so forth. The binary point’s rightmost digits (or bits) have weights of 2^-1, 2^-2, 2^-3, and so on.
...
As much as as this represntation seems easy and intuitive, it has some limitations. Some fractional numbers can't be represented without loosing precision. for example try solving the number 0.1. the binary fractional part tend to infinity. Therefore in numerical systems, The CPU would truncate some bits so the number will fit in memory and that is waht loosing precision is.
````
0.1 (decimal) = 0.000110011001100110011... (repeating)
```` 

### 3.3 fixed point multiplication (ARM)
Let us take a code example in arm cortex M0 compiled with arm-none-eabi-gcc. We added the "naked" attribute to eliminate the prologue and epilogue sequences and focus only in our useful part.
````
...
.L3:
        .word   954437177
````
Breaking this mythical calculation, we can clearly see the UMULL instruction suggesting the operation of division is transformed into a multiplication.
...
````
Second step: Load the input number and magic number in registers
````
r3 = num
r2 = 954437177
````
...
````
## 4. Consequences:
After testing differing examples this what we noticed:
1. magic number's multiplier ( 2^33 ) may vary according the input's type.
2. The higher the magic number, the better precision in the result for high input values. 

## 5. Buffer sizes must be multiple of 2:
The matter we have been presenting would directly affect the usage of ring buffers or circular buffers. In fact using circular buffers is related with divsion.
![image](https://github.com/user-attachments/assets/e7f80d52-f8cb-4935-a6a0-9744dfbaf65f)

If the index exceeds the size of the buffer, we would find the new index by taking the rest of the division of the index/buffer_size
### 5.1 Circular buffer index
The modulo operation is related to the division operation. ARM compiler solves the mod operation following this a % b = a - (a/b)*b. a/b is calculated via the fixed point multiplication explained earlier. 
````
...
.L9:
        .word   -2004318071
````
### 5.2 buffer size must be mulitiple of two:
If we choose a buffer size a multiple of two would result to a lot les instruction and this is the output:
````
...
````
The power of 2 size requirement for circular buffers enables efficient wrapping using bitwise AND instead of modulo (%) operation. Here's why:
